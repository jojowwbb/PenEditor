...
function parseOutput(place, arr, type) {
	var index = -1;
	var result = '<div class="log-wrap"><span class="log-type__'
		.concat(type, '">')
		.concat(new Date().toLocaleString(), '</span><span class="log-type__')
		.concat(type, '"> [ ')
		.concat(type, " ]:</span>");
...
function genOutput(str, rest, type) {
	...
	if (typeof str === "string") {
		...
		output = '<div class="log-wrap"><span class="log-type__'
			.concat(type, '">')
			.concat(new Date().toLocaleString(), '</span><span class="log-type__')
			.concat(type, '"> [ ')
			.concat(type, ' ]:</span><span class="log-str">')
			.concat(objstr, "</span></div>");
	}
